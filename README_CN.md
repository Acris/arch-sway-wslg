...
`paru` 展示的 AUR PKGBUILD 前请先审阅。拒绝桌面条目屏蔽不会移除任何已有的同名文件；旧屏蔽请按[卸载](#卸载)一节的说明手动移除。
...
安装时选择的浏览器记录在 `~/.config/arch-sway-wslg/browser` 中，并在会话内导出为 `BROWSER`，供支持该变量的应用使用。可以通过编辑该文件
（包含单个可执行文件名）或导出 `BROWSER` 来修改此变量。
